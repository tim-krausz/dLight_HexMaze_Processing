...
#load signals from matlab script. Start time is when pulse was sent from arduino
signals = scipy.io.loadmat(datepath+'signals.mat',matlab_compatible=True)
ref = signals['ref']
sig1 = signals['sig1']
#sig2 = signals['sig2']
loc = signals['loc']
location = list(np.concatenate(loc))
location = ''.join(location)
visits = signals['visits'][0]
...
#downsample phot data
SR = 10000 #sampling rate of photometry system
Fs = 250
visits = np.divide(visits,SR/Fs)
visits = visits.astype(int)

sig1 = sig1[0][::int(SR/Fs)]
ref = ref[0][::int(SR/Fs)]
if datepath == '/Volumes/Tim/Photometry/IM-1532/12132022/':
    ref = ref[:-1000]
    sig1 = sig1[:-1000]
...
#shiftVisLeftLate = bool(int(input("only shift visinds left for position detection? (1/0)")))
#shiftVisRightLate = bool(int(input("only shift visinds right for position detection? (1/0)")))
#if shiftVisLeftLate:
#    vis4pos = visits[1:]
#elif shiftVisRightLate:
#    vis4pos = np.concatenate([[0],visits])
#else:
#    vis4pos = visits
vis4pos = visits
x,y,vel,framestamps,acc,dlc_pos = align_pos_to_visits(Fs,vis4pos,datepath,
    phot_dlc=phot_dlc,filecount=filecount,gaus_smooth=usePosSmoothing,sigma=gausSigma)
...
data[:,2] = dff1.T[0] #green
data[:,3] = 0#dff2.T[0] #red
data[:,16] = sig1
data[:,17] = 0#sig2 # 565
data[:,11] = ref.T[0]
block = 1
